import { PrismaClient, Rol, Disciplina, Simulador, EstadoCampeonato, EstadoCarrera, EstadoInscripcion, CanalChat, TipoNotificacion, PlanSuscripcion } from '@prisma/client'
...
async function main() {
  ...
  await prisma.cocheDesbloqueado.deleteMany()
  await prisma.coche.deleteMany()
  ...
  const PLAN_PRECIOS_SEED: Record<string, number> = { ROOKIE: 5, AMATEUR: 10, PRO: 18, ELITE: 25 }
  const PLAN_ORDER_SEED: PlanSuscripcion[] = ['ROOKIE', 'AMATEUR', 'PRO', 'ELITE']

  async function crearSuscripcionSeed(userId: string, plan: PlanSuscripcion) {
    const fechaRenovacion = new Date()
    fechaRenovacion.setMonth(fechaRenovacion.getMonth() + 1)

    await prisma.suscripcion.upsert({
      where: { userId },
      update: { plan, estado: 'ACTIVA', precioMensual: PLAN_PRECIOS_SEED[plan], fechaRenovacion },
      create: { userId, plan, estado: 'ACTIVA', precioMensual: PLAN_PRECIOS_SEED[plan], fechaRenovacion },
    })

    // Desbloquear coches correspondientes
    const planIdx = PLAN_ORDER_SEED.indexOf(plan)
    const planesValidos = PLAN_ORDER_SEED.slice(0, planIdx + 1)
    const cochesParaDesbloquear = cochesCreados.filter(c => planesValidos.includes(c.planMinimo as PlanSuscripcion))

    await prisma.cocheDesbloqueado.createMany({
      data: cochesParaDesbloquear.map(c => ({ userId, cocheId: c.id })),
      skipDuplicates: true,
    })
  }
  ...
  for (const p of pilotos.slice(0, 5)) {
    await crearSuscripcionSeed(p.id, PlanSuscripcion.ROOKIE)
  }

  // Pilotos 5-7: Pro
  for (const p of pilotos.slice(5, 8)) {
